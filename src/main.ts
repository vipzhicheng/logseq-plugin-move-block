...
const handler = async (e: any) => {
  ...
  if (block && block.content) {
    ...
    if (srcBlock) {
      if (srcBlock.content) {
        await logseq.Editor.insertBlock(srcBlock.uuid, `((${block.uuid}))`, { sibling: true });
      } else {
...
async function main() {
  logseq.Editor.registerSlashCommand( `Copy block ref to today's journal`, handler );
  logseq.Editor.registerBlockContextMenuItem(`Copy block ref to today's journal`, handler);
}
